...
class latent_chord:
    ...
    @property
    def full_name(self):
        return self.triad+self.octave+'_'+self.volume+'_'+self.example
    ...
    def from_file(cls, path, filename):
        if filename[-4:]=='.wav':
            audio_origin = import_audio2(path+filename)[0,:]
            filename = filename[0:-4]
        else:
            audio_origin = import_audio2(path+filename+'.wav')[0,:]
        mel = self.spec_helper.waves_to_melspecgrams(audio_origin.reshape([1,64000,1]))
        latent, _ = self.model.encode(mel)
        new_latent_chord = latent_chord(latent)
        chord_octave,new_latent_chord.volume,new_latent_chord.example = filename.split('_')
