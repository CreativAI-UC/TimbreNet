...
R_LOSS_FACTOR_SEQUENCE = [[1,15],[0.2,85],[0.1,50],[0.1,50],[0.1,50]]

# GLOBAL_PARENT_TRAIN_ID = 'ID_2020'                        #HERE SET THE PARENT ID, PUT NONE IF COMPLETLY NEW MODEL
# R_LOSS_FACTOR_SEQUENCE = [[0.2,85],[0.1,50]]                   #[[r_loss_factor, number_epochs],[r_loss_factor, number_epochs],…]
...
for R_LOSS_FACTOR_PAIR in R_LOSS_FACTOR_SEQUENCE:
    ...
    #ELSE (TRAIN STARTS FROM OTHER TRAINED MODEL) COPY THE DATA FROM PARENT TRAIN AND UPDATE  hparams
        ...
        mode = 'load'
        with open(os.path.join('./run/{}'.format(GLOBAL_PARENT_TRAIN_ID), 'train_params.txt')) as f:
        ...
        copy_tree('./run/{}'.format(GLOBAL_PARENT_TRAIN_ID), RUN_FOLDER)
